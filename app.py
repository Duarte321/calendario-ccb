...
def gerar_pdf_buffer(ano, lista_eventos):
    ...
    for mes in range(1, 13):
        ...
        pdf.cell(0, 15, f"{NOMES_MESES[mes].upper()} {ano}", align="C", ln=1)
        pdf.ln(5)
        
        # Filtra eventos desse mês
        dados = calcular_eventos(ano, lista_eventos)
        eventos_mes = []
        for dia in range(1, 32):
            chave = f"{ano}-{mes}-{dia}"
            if chave in dados:
                eventos_mes.append(f"Dia {dia}: {', '.join(dados[chave])}")
        
        pdf.set_font("Arial", size=11)
        if eventos_mes:
            for ev in eventos_mes:
                pdf.multi_cell(0, 7, ev.encode('latin-1', 'replace').decode('latin-1'))
        else:
