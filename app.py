...
if st.session_state.nav=="Agenda":
    ...
    with center:
        ...
        p=['<div class="cal-grid">']+[f'<div class="cal-head">{x}</div>' for x in DIAS_SEMANA_CURTO]
        for semana in calendar.monthcalendar(st.session_state.ano_base,mes):
            for dia in semana:
                ...
                p.append('</div>')
        p.append('</div><div class="legend"><span class="legend-box"></span>Dias com ensaio</div>');st.markdown("".join(p),unsafe_allow_html=True);st.markdown('</div>',unsafe_allow_html=True)
    with right:
